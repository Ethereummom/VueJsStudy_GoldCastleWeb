<template>
    <!-- <div>
        <div class = "section" id = "products">
            <div class = "container">
                <div>
                    <div class = "products">
                        <img :src= "product.pphoto" alt = "plh">
                        <p>{{product.pname}}</p>
                        <p>{{product.pcontent}}</p>
                        <p>{{product.pprice}}</p>
                    </div>
                    <br> <a href = "/products">상품목록으로 돌아가기</a>
                </div>
            </div>
        </div>
    ...
    <div id="spStrap" sp-contents class="initialize" spstrap>
        <div class="mo-4BC8F294A9 initialize" sp-edit="box" sp-sort-nodrop="fixed">
            <div sp-edit="box" sp-size sp-sort-nodrop="element" style="padding:4rem 0;" class="initialize">

                <div sp-edit="text" style="padding:1rem 0;text-align:center;" class="initialize" area="before" draggable="false"><span sp-font="13" style="color:#333;" draggable="false"><b draggable="false">D E T A I L . V I E W</b></span></div>

                <div sp-edit="box" style="padding:6rem 0 0 0;text-align:center;" class="initialize" area="before">
                    <a href="#!" sp-edit="image_button" sp-resize style="width:80%;" class="initialize"><img :src="product.pphoto" sp-edit="image" style="width:100%;" class="initialize"></a>
                </div><div sp-edit="text" style="padding:0;text-align:center;" class="initialize" area="before"><span sp-font="8" style="color:#888;">{{product.pdescription}}</span><br><span sp-font="8" style="color:#888;"><p>{{product.pdescription}}</p></span></div><div sp-edit="text" style="line-height:2;padding:3rem 0 0 0;text-align:center;" class="initialize" area="after"><span sp-font="9" style="color:#fff;background-color:#444;"><b>&nbsp; K R W {{product.pprice}} &nbsp;</b></span></div>
...
export default{
    ...
    methods : {
        fetchProductDetails(pid){
            axios.get(`http://localhost:8070/api/v1/product/products/${pid}`)
            .then(response => {
                this.product = response.data;
            })
            .catch(error => {
                console.error('상품 상세 정보를 불러올 수 없었습니다.', error);
            });
        },
